...
class _StockPriceItemState extends ConsumerState<StockPriceItem> {
  ...
  void initState() {
    ...
    ref
        ...
        .getStockPriceBySymbol(widget.stock.symbol)
        .then((r) {
          ...
          builder: (context) {
            return AlertDialog(
              ...
              actions: [
                TextButton(
                  onPressed: () {
                    ref.read(myViewmodelProvider.notifier).deleteFromSymbol(widget.stock.id);
                ...
                try {
                  ...
                  return Row(
                    children: [
                      ...
                      Text(
                          "${((price.p - previousCloseValue) / previousCloseValue * 100).toStringAsFixed(2)}%")
                    ],
                  );
                } catch (e) {
                  print("socket: $e");
                  ...
                  return Row(
                    children: [
                      Text(
                          "${double.parse(previousValue).toStringAsFixed(2)}\$"),
                      Text(
                          "${((double.parse(previousValue) - previousCloseValue) / previousCloseValue * 100).toStringAsFixed(2)}%")
                    ],
                  );
                }
              },
